...
public class beta {
	...
	//simple wrapper for System.out.println method
	private static void print(Object o){
		System.out.println(o);
	}
	
	//logSum method that calculates summing of exponentiated logarithmic numbers
	private static double logSum(double d1, double d2){
		
		if(d1>d2) {
			return d1 + Math.log1p(Math.exp(d2 - d1));
		}else if(d1<d2){
			return d2 + Math.log1p(Math.exp(d1 - d2));
		}else{
			return d1+Math.log1p(1);
		}
		
	}
	//simple wrapper for Math.log method
...
/************************Testing on the dev Matrix******************************/
		...
		double aji=0;
		double pol=0.0;
		print("Log Probability of Each Sentence:");
		//iterate through each sequence
		for (int k=0;k<devMatrix.size();k++){               
			atArray = new Double[8];
			attArray = new Double[8];
			//iterate through each symbol
			for(int t=devMatrix.get(k).size()-1;t>=0;t--){
				Arrays.fill(attArray, 0.0);
				if(t==devMatrix.get(k).size()-1){
					//Traverse according to the sequence in emitIndexR
					for(int i=0;i<emitIndexR.size();i++){
						Arrays.fill(atArray, 0.0);
					}
				}else{
					//iterate through destination states
					for(int j=0;j<emitIndexR.size();j++){
						//iterate through original states
						for(int i=0;i<emitIndexR.size();i++){
							aji=Double.valueOf(transMatrix[j][i]);
							if(i==0){
								attArray[j]=atArray[i]+log(aji)+log(Double.valueOf(emitMatrix[i][emitIndexC.get(devMatrix.get(k).get(t+1))]));
							}else{
								attArray[j]= logSum(attArray[j],atArray[i]+log(aji)+log(Double.valueOf(emitMatrix[i][emitIndexC.get(devMatrix.get(k).get(t+1))])));
							}
						}
					}
			...
			pol = attArray[0]+log(initProb[0])+log(Double.valueOf(emitMatrix[0][emitIndexC.get(devMatrix.get(k).get(0))]));
			for(int i=1;i<attArray.length;i++){
				pol = logSum(pol,attArray[i]+log(initProb[i])+log(Double.valueOf(emitMatrix[i][emitIndexC.get(devMatrix.get(k).get(0))])));
			}
			print(pol);
		}
	}
