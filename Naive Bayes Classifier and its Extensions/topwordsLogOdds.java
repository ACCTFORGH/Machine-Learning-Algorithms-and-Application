...
public class topwordsLogOdds{
	...
	public static void main(String[] args) {
		...
		//Calculate LogOdds
		HashMap<String, Double> conLog = new HashMap<String, Double>();
		HashMap<String, Double> libLog = new HashMap<String, Double>();
		...
		for(String key: keys){
			...
			if (conKeys.contains(key)){
				...
				pc = calLogProb(0,conPosition,volcSize,q);
			}
			
			if (libKeys.contains(key)){
				pl = libProb.get(key);
			}else if (keys.contains(key)){
				pl = calLogProb(0,libPosition,volcSize,q);
			}
			
			conLog.put(key, (pc-pl));
			libLog.put(key, (pl-pc));
		...
		conList = sortMap(conLog,20);
		libList = sortMap(libLog,20);
