...
public class nb {
	...
	public static double calLogProb(int nk, int n, int volc, int q){
		double prob=0;
		double denom = (double)(n+q*(volc+1));
		double numer = (double)(nk+q);
		...
		try{
			...
			for(String filename:testFile){
				...
				if (libScore > conScore){
					...
					System.out.println("L"); //+" "+ libScore+" "+conScore);

				}else if(libScore < conScore){
					if (filename.equals("lib")){
						errorCounter += 1;
					}
					System.out.println("C");//+" "+libScore+" "+conScore);
				}
				counter += 1;
			}
			DecimalFormat decimalformat = new DecimalFormat("#.0000");
			System.out.println("Accurary: "+ decimalformat.format((1-(double)errorCounter/(double)counter)));
